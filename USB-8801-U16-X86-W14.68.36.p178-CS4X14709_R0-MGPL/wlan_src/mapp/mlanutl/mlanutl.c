...
process_getstalist(int argc, char *argv[])
{
	...
	for (i = 0; i < list->sta_count; i++) {
		...
		printf("\nPower mfg status: %s\n",
			...
			0) ? "active" : "power save");
		printf("Mode: %s\n",
		       (list->client_info[i].bandmode ==
			BAND_B) ? "11b," : (list->client_info[i].bandmode ==
					    BAND_G) ? "11g," : (list->
								client_info[i].
								bandmode ==
								BAND_GN) ?
			  ...
			  wlan_ioctl_user_scan_cfg *scan_req)
{
	...
	for (curr = scan_list_head, idx = 0;
		 ...
		 */
		printf(" %c%c%c%c%c%c%c%c | ", bss_info->cap_info.ibss ? 'A' : 'I', bss_info->priv_cap,	/* P (WEP), W (WPA), 2 (WPA2) */
...
process_tx_ampdu_prot_mode(int argc, char *argv[])
{
	...
	if (data.mode == TX_AMPDU_RTS_CTS)
		...
		printf("CTS-2-SELF\n");
	else if (data.mode == TX_AMPDU_DISABLE_PROTECTION)
		printf("Disabled\n");
	printf("\n");
...
*/
static int
process_txwatchdog(int argc, char *argv[])
{
	t_u8 *buffer = NULL;
	struct eth_priv_cmd *cmd = NULL;
	struct ifreq ifr;
