			      ...
			      IN custom_ie *ie_data, IN t_u16 idx)
{
	...
	for (index = 0;
		...
		if (MLAN_CUSTOM_IE_AUTO_IDX_MASK != idx &&
		    (idx <
		     MIN(MAX_MGMT_IE_INDEX, pmadapter->max_mgmt_ie_index)))
			index = idx;
			 ...
			 IN pmlan_ioctl_req pioctl_req)
{
	...
	if (pioctl_req->action == MLAN_ACT_GET) {
		...
		debug_info->num_cons_assoc_failure =
	...
	if ((misc->param.cust_ie.len == 0) ||
		...
		while (ioctl_len > 0) {
			...
			if (MLAN_CUSTOM_IE_AUTO_IDX_MASK == index) {
				...
				if (index >=
				    MIN(pmadapter->max_mgmt_ie_index,
					MAX_MGMT_IE_INDEX)) {
					PRINTM(MERROR,
			       ...
			       IN t_u32 payload_len, IN RxPD *prx_pd)
{
	...
	case SUBTYPE_DEAUTH:
		...
		if (priv->uap_host_based & UAP_FLAG_HOST_MLME) {
			if (!memcmp
			    (pmadapter, pieee_pkt_hdr->addr3, priv->curr_addr,
			     MLAN_MAC_ADDR_LENGTH)) {
				PRINTM_NETINTF(MMSG, priv);
				PRINTM(MMSG,
			       ...
			       IN pmlan_ioctl_req pioctl_req)
{
	...
	if (pmpriv->media_connected != MTRUE) {
		if (pmpriv->config_bands & (BAND_B | BAND_G)) {
			...
			PRINTM(MINFO, "Invalid Band 0x%x\n",
			       pmpriv->config_bands);
		}
 ...
 */
mlan_status
wlan_misc_ioctl_tx_ampdu_prot_mode(IN pmlan_adapter pmadapter,
				   IN pmlan_ioctl_req pioctl_req)
{
	mlan_private *pmpriv = pmadapter->priv[pioctl_req->bss_index];
	...
	if (pioctl_req->action == MLAN_ACT_SET)
		...
		cmd_action = HostCmd_ACT_GEN_GET;
	ret = wlan_prepare_cmd(pmpriv,
			       HostCmd_CMD_TX_AMPDU_PROT_MODE,
			       cmd_action,
			       0,
			       (t_void *)pioctl_req,
