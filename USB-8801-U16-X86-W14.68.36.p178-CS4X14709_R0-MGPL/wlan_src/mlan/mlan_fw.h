...
#define IS_11N_ENABLED(priv) ((priv->config_bands & BAND_GN) && priv->curr_bss_params.bss_descriptor.pht_cap && !priv->curr_bss_params.bss_descriptor.disable_11n)
...
/** Check if supplicant is supported by firmware, old chip did not need to check fw cap info for embedded supplicant support */
#define IS_FW_SUPPORT_SUPPLICANT(_adapter)    (1)
