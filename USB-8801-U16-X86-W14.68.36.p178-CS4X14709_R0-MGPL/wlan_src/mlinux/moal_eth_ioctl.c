...
woal_priv_setuserscan(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
	...
	memcpy((char *)&scan_cfg,
	       respbuf + strlen(CMD_NXP) + strlen(PRIV_CMD_SETUSERSCAN),
...
woal_priv_getscantable(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
	...
	memcpy(&scan_start,
	       respbuf + strlen(CMD_NXP) + strlen(PRIV_CMD_GETSCANTABLE),
	       sizeof(scan_start));
	if (scan_start)
...
woal_priv_atim_window(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
	...
	header_len = strlen(CMD_NXP) + strlen(PRIV_CMD_ATIM_WINDOW);
	req = woal_alloc_mlan_ioctl_req(sizeof(mlan_ds_bss));
	if (req == NULL) {
		ret = -ENOMEM;
		goto done;
...
woal_priv_adhoc_aes(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
	...
	header_len = strlen(CMD_NXP) + strlen(PRIV_CMD_ADHOC_AES);
 ...
 */
static int
woal_priv_txwatchdog(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
	int ret = 0;
	int header_len = 0;
	int user_data_len = 0;
			     ...
			     t_u32 respbuflen)
{
	...
	if (strlen(respbuf) == header_len) {
		...
		if (data[0] < TX_AMPDU_RTS_CTS ||
		    data[0] > TX_AMPDU_DISABLE_PROTECTION) {
...
woal_priv_acs(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
	...
	memset(data, 0, sizeof(data));
	header_len = strlen(CMD_NXP) + strlen(PRIV_CMD_ACS_SCAN);
	...
	if (strncmp(buf, CMD_NXP, strlen(CMD_NXP)) == 0) {
		...
		if (strnicmp
			...
			goto handled;
			   ...
			   (buf + strlen(CMD_NXP), PRIV_CMD_ARPFILTER,
			...
			goto handled;
			   ...
			   (buf + strlen(CMD_NXP), PRIV_CMD_ATIM_WINDOW,
			...
			goto handled;
			   ...
			   (buf + strlen(CMD_NXP), PRIV_CMD_ADHOC_AES,
			...
			goto handled;
			   ...
			   (buf + strlen(CMD_NXP), PRIV_CMD_TX_AGGR_CTRL,
			...
			goto handled;
			   ...
			   (buf + strlen(CMD_NXP), PRIV_CMD_AUTO_TDLS,
			...
			goto handled;
			   ...
			   (buf + strlen(CMD_NXP), PRIV_CMD_TDLS_IDLE_TIME,
			...
			goto handled;
			   ...
			   (buf + strlen(CMD_NXP), PRIV_CMD_ACS_SCAN,
