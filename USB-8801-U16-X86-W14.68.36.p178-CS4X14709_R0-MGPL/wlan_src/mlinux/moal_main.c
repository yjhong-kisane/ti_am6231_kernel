...
/** Driver version */
char driver_version[] =
	"USB8801-%s-C" KERN_VERSION "14" MLAN_RELEASE_VERSION
	"-GPL" "-(" "FP" FPNUM ")"
#ifdef	DEBUG_LEVEL2
	"-dbg"
#endif
	...
	for_each_property_of_node(dt_node, prop) {
		if (!strncmp(prop->name, "drv_mode", strlen("drv_mode"))) {
			...
			if (!of_property_read_u32(dt_node, prop->name, &data)) {
			 ...
			 (prop->name, "sched_scan", strlen("sched_scan"))) {
			if (!of_property_read_u32(dt_node, prop->name, &data)) {
				PRINTM(MERROR, "sched_scan = 0x%x\n", data);
				sched_scan = data;
			}
		}
	}
		      ...
		      mlan_buffer *pmbuf, struct sk_buff *skb)
{
	...
	if (IS_SKB_MAGIC_VLAN(skb)) {
		...
		case __constant_htons(ETH_P_IPV6):
			tid = (IPTOS_PREC(SKB_TIDV6(skb)) >> IPTOS_OFFSET);
			PRINTM(MDAT_D,
...
#ifdef STA_CFG80211
	if (priv->sched_scan_workqueue) {
		flush_workqueue(priv->sched_scan_workqueue);
		destroy_workqueue(priv->sched_scan_workqueue);
		priv->sched_scan_workqueue = NULL;
	}
#endif
...
#define NAPI_BUDGET     64
	...
	device_init_wakeup(handle->hotplug_device, true);
...
MODULE_PARM_DESC(cntry_txpwr,
		 "0: disable (default), 1: enable set country txpower table");
